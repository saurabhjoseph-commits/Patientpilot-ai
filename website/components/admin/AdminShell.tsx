...
export default function AdminShell({
  ...
  return (
    <div className="min-h-screen bg-slate-100 lg:flex">
      <div className="hidden lg:block lg:shrink-0"><Sidebar canViewClinics={canViewClinics} canViewDoctors={canViewDoctors} canManageGlobal={canManageGlobal} /></div>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 transform transition-transform lg:hidden ${open ? "translate-x-0" : "-translate-x-full"}`}>
        <Sidebar canViewClinics={canViewClinics} canViewDoctors={canViewDoctors} canManageGlobal={canManageGlobal} onNavigate={() => setOpen(false)} />
      </aside>

...
const steps = ["Basic Information", "Contact", "Address", "Business Hours", "AI Settings", "Review"];
...
export default function NewClinicWizard() {
  ...
  const submit = async () => {
    ...
    router.replace("/admin/clinics?created=1"); router.refresh();
  };
  const content = [
    ...
    <ClinicReviewStep key="review" value={{ basicInfo: basic, contact, address, businessHours, aiSettings }} submitting={submitting} onBack={() => setStep(4)} onCreate={submit} />,
  ];

...
export default function NewClinicWizard() {
  ...
  return <section className="space-y-6"><WizardStepper currentStep={step} completedSteps={Array.from({ length: step }, (_, index) => index)} /><p className="text-sm font-medium text-blue-600">Step {step + 1} of {steps.length} — {steps[step]}</p>{error && <p role="alert" className="rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-700">{error}</p>}{content[step]}{step < steps.length - 1 && <WizardNavigation canGoBack={step > 0} canGoNext onBack={() => setStep((current) => Math.max(0, current - 1))} onNext={next} onSubmit={submit} canSubmit={false} loading={submitting} />}</section>;
}

...
import DoctorAdministrationControls from "@/components/admin/DoctorAdministrationControls";
...
export default async function EditDoctorPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ clinicId?: string }> }) {
  ...
  return <main className="mx-auto max-w-3xl space-y-6"><div><h1 className="text-3xl font-bold">Edit doctor</h1><p className="mt-1 text-sm text-slate-500">Working schedule is configured in the scheduling module after G4 is deployed.</p></div><DoctorForm doctor={doctor} clinicId={canManageDoctorsGlobally(identity) ? clinicId : undefined} services={services ?? []} assignedServiceIds={assigned} canAssignServices={identity.permissionCodes.includes(Permissions.DoctorsAssignServices)} /><DoctorAdministrationControls doctorId={doctor.id} clinicId={canManageDoctorsGlobally(identity) ? clinicId : undefined} fullName={doctor.fullName} status={doctor.status} canDeactivate={identity.permissionCodes.includes(Permissions.DoctorsDeactivate)} canDelete={canManageDoctorsGlobally(identity) && identity.permissionCodes.includes(Permissions.DoctorsDelete)} /></main>;
}

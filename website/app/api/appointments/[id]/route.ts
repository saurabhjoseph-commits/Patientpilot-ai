...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  } catch (error) { if (isAppointmentSlotConflict(error)) return NextResponse.json({ code: error.code, message: error.message }, { status: 409 }); return NextResponse.json({ message: error instanceof Error ? error.message : "Unable to update appointment." }, { status: 400 }); }
}

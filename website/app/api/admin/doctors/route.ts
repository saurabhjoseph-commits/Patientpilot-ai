...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const result = await createDoctorService().list({ clinicId: resolveAdminClinic(authorization).clinicId, query: params.get("q")?.trim() || undefined, status: status === "active" || status === "inactive" ? status : undefined, sort: params.get("sort") === "created" ? "created" : "name", page: integer(params.get("page"), 1), pageSize: integer(params.get("pageSize"), 20) });
    return NextResponse.json(result);
  } catch {
    return NextResponse.json({ message: "Unable to load doctors." }, { status: 500 });
  }
}

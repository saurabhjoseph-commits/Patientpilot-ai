...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const assignments = Array.isArray(body) ? body : [];
    if (!assignments.every(valid)) return NextResponse.json({ message: "Invalid service assignment." }, { status: 400 });
    await createDoctorService().setServices(resolveAdminClinic(authorization).clinicId, (await params).id, assignments);
    return NextResponse.json({ success: true });
  } catch (error) { return NextResponse.json({ message: error instanceof Error ? error.message : "Unable to assign services." }, { status: 400 }); }
}

...
test("H2.1 appointment linkage validates same-clinic doctor, service, room, and duration", () => {
  ...
  assert.match(resolver, /Selected service does not belong to this clinic/); assert.match(resolver, /Selected doctor does not belong to this clinic/); assert.match(resolver, /Selected doctor is not assigned to this service/); assert.match(resolver, /Selected room does not belong to this clinic/); assert.match(resolver, /let durationMinutes = service\.default_duration_minutes/); assert.match(resolver, /custom_duration_minutes \?\? durationMinutes/); assert.doesNotMatch(resolver, /linkage\.durationMinutes \?\? service\.default_duration_minutes/);

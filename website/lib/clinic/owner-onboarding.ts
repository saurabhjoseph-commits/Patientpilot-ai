...
export async function assertOwnerOnboardingReady(owner: ClinicOwnerAccount): Promise<void> {
  ...
  for (let page = 1; page <= 100; page += 1) {
    ...
    if ((data.users ?? []).some((user) => user.email?.toLowerCase() === email)) throw new OwnerOnboardingError("That owner email already belongs to an Auth account. Use a different email or resolve the existing account first.");
...
export async function resendOwnerActivation(clinicId: string): Promise<OwnerOnboardingRecord> {
  ...
  if (!record.auth_user_id) throw new OwnerOnboardingError("The owner account was not created. Retry onboarding from the clinic details page.");
...
export async function retryOwnerProfileSetup(clinicId: string): Promise<OwnerOnboardingRecord> {
  const record = await requiredRecord(clinicId);
  if (!record.auth_user_id) throw new OwnerOnboardingError("No Auth identity exists yet. Resend the activation invitation instead.");
...
async function sendInvitation(record: OwnerOnboardingRecord): Promise<OwnerOnboardingRecord> {
  ...
  return updateRecord(record.id, { status: "invited", invitation_sent_at: new Date().toISOString(), last_attempt_at: new Date().toISOString(), failure_code: null });
}

...
-- Adds emergency configuration to the existing atomic clinic-onboarding RPC.
-- No table or column is created; public.clinic_settings.emergency_rules exists
...
begin
  ...
  insert into public.clinic_settings (clinic_id, greeting, office_hours, scheduling_rules, emergency_rules, voice, language)
  values (v_clinic_id, p_greeting, p_office_hours, p_scheduling_rules, p_emergency_rules, p_voice, p_language);

  return query select v_clinic_id;
end;
$$;

revoke all on function public.create_clinic_with_settings(text,text,text,text,text,text,text,text,text,text,text,jsonb,text,text,jsonb,jsonb) from public;
